...
### 加速档与供电（2026-09-23 实测结论）

电池单独供电（含 1.6V 台电源，实测电池端 1.56V 不跌落）下，**240→480MHz 跳变会卡死**（首页后失去响应、约 105-118mA 空转）；USB 5V 供电正常，标准档（240MHz）电池供电正常。已排除电池内阻与 DOUBLE_FETS 配置（静态/动态均试）——判定为设备供电通路无法承受该跳变。
固件已做兜底：**无外接电源时强制标准档**（设置页显示实际档位并提示 `[需外接电源]`）。电池供电请使用标准档。
...
## 为什么操作手感像"比原版慢"，但实际并不慢

**体感来自"每次操作的延迟"，不是"算得快不快"。**

| | 原厂固件 | ExistOS |
|---|---|---|
| 程序存储 | 内部 flash 直接执行（XIP）、工作集全常驻 | HP39GII **没有内部程序 flash**，只能 NAND 启动 → OSLoader + 虚拟内存**按需分页** + FTL |
| 首次访问不在缓存的代码/数据 | 无此开销 | 需从 NAND 取页（微秒~毫秒级，是体感差异的主因） |
| 按键响应路径 | 单任务紧循环，亚毫秒级 | 多任务系统，UI 轮询（约 10~30ms 一轮），且可能触发换页 |

所以"感觉慢"主要出现在**刚开机、刚进应用、长时间空闲后的第一次操作**（冷缓存），以及频繁切换应用时；连续操作与长时间计算反而更快：

| 8×8 皇后（同一算法） | 原厂 PPL（编译型） | ExistOS Python app |
|---|---|---|
| 耗时 | 约 1.5 s @ 80MHz | **1.22 s @ 240MHz / 0.70 s @ 480MHz** |

- CPU 主频是原版的 3~6 倍（240/480MHz vs 80MHz），且原版**不开放 CAS**（CAS 语法编译进固件但界面未提供）
- 原版快的本质是"裸机 + 全常驻"：没有换页、没有多任务、没有 OS，单次按键延迟天然更小
- 一句话：**吞吐（算得快）我们更快，单次操作首延迟（手感）裸机更小**——两者不是一回事
