...
说明：系统无软件低电压关机逻辑；单节 AAA 在约 1.2V 开路电压下"自动关机"实为 硬件 brownout 复位（阈值 0.79V）在负载压降下的表现。长时间高负载（加速模式/充电）建议使用新电池或外接电源；电流需求较高时推荐**多节电池并联**使用，或采用 **AAA 锂电池**（1.5V 恒压、大电流输出强、低温性能好）——单节镍氢难以满足启动峰值电流。
