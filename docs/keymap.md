...
| X² / X^Y | `^2` / `^` | `sqrt(` / `^(1/` |
| ÷ / − / × / + | `/` / `-` / `*` / `+` | `^-1` / `polar_complex(` / `!` / 空格 |
| **a b/c** | **`exact(`**（2026-09-16 修正，原被转为 F7 菜单） | `"` |
| X,T,θ,N | `x`（或 `t`） | `10^` |
...
| 键（面板标注） | 编辑器里实际 | 状态 |
|----------------|--------------|------|
| a b/c | ~~F7 菜单（线性代数）~~ | **已移除**（2026-09-16，回归 `exact(`） |
| SHIFT+4（Matrix） | F8 菜单（复数） | 与标注不符（待裁决） |
| SHIFT+7（List） | F9 菜单（算术） | 与标注不符（待裁决） |
| SHIFT+1（Prgm） | F12 菜单（编程） | 相符 |
| SHIFT+−（∠） | F13 菜单（几何） | 相符 |
| （MIXEDFRAC→F10 / FD→F11 / CHAR_FRAC→F14） | 无物理键绑定，不触发 | — |
