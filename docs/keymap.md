...
**Shift+0 = 命令输入自动补全**（2026-09-19）：输入前缀后按 Shift+0，先在 355 条命令目录（中文名显示）与 1552 条内置函数名中前缀匹配；输入完整命令名（如 sin/ln）优先精确匹配；唯一匹配直接补全为 `命令(`（程序结构如 while/if 插入原文本）；多匹配弹出 doMenu 选择菜单；无匹配不动作。仅在 Console 输入行生效。
