...
- **底栏**：F1 符号 ｜ F2 清屏 ｜ F3 取消 ｜ F4 运行脚本 ｜ **F5 run（执行输入）** ｜ F6 文件
...
- **ENT 只换行**，不执行——可以连续输入多行代码（`for`/`def` 等直接换行书写，无需空行结束）
- 输入完成后按 **F5（run）** 执行**自上次运行以来输入的全部内容**（结果输出显示在其下方，并出现新的 `>>>` 提示符）
- **单行**表达式/语句用 `single` 模式执行：表达式会**显示结果值**（如 `2+3` → `5`）；多行代码用 `exec` 模式执行（如需输出请用 `print()`）
