...
/*
    ...
    //HW_POWER_MINPWR.B.DOUBLE_FETS = 1; // 2026-09-23 实验结论：加倍 FET 无法解决 240→480 跳变卡死，已撤
...
/*
    ...
    portDelayus(100);
    // 手册 29.2.2.2 推荐瞬态优化位（2026-09-03 启用）：TOGGLE_DIF / EN_CM_HYST / EN_RCSCALE=1
    BF_SET(POWER_LOOPCTRL, TOGGLE_DIF);
    BF_SET(POWER_LOOPCTRL, EN_CM_HYST);
    BF_WR(POWER_LOOPCTRL, EN_RCSCALE, 0x1);
