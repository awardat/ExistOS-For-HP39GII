...
把 `*.py`（或整个 `samples/` 子目录）放到设备的 **`/xcas/`** 目录——
KhiCAS 文件浏览器的根目录就是它（串口日志可见路径映射 `\\fls0\ -> /xcas/`），
可以进入其下的子目录选择脚本。用你惯用的方式（F2 复制 / edb / 文件管理器）。

> 提示：设备 `/xcas/` 下已有 KhiCAS 自身的文件（session.xw 等），属正常。
