...
static void pendNewline(void) { // ENT：换行并按 Python 缩进规则自动缩进
    int ls = pendLineStart();
    int ind = 0;
    while (ls + ind < pendCur && pend[ls + ind] == ' ') ind++; // 本行前导空格
    int last = pendCur - 1;
    while (last >= ls && (pend[last] == ' ' || pend[last] == '\t')) last--;
    int nextInd = 0;
    if (last >= ls) nextInd = (pend[last] == ':') ? ind + 4 : ind; // ':' 结尾 +4；非空行沿用
    pendInsert('\n');
    for (int i = 0; i < nextInd; i++) pendInsert(' ');
}
        ...
        if (kp) {
            if (key == KEY_SHIFT) {
                ...
                if (uiMode == UI_SYMB) {
                    ...
                    pendNewline(); // ENT 只换行（执行请按 F5 run）；':' 结尾自动缩进 4 空格
