...
void pageUpdate() {
    ...
    if (curPage == 3) {
        ...
        if (page3Subpage == 0) {
            ...
            line = 3;
            uidisp->draw_printf(DISPX + (DISPW - 120) / 2, DISPY - 8 + 16 * line++, 16, 64, 255, "HP39GII\xbf\xaa\xd4\xb4\xb9\xcc\xbc\xfe");
            uidisp->draw_printf(DISPX + 16, DISPY - 4 + 16 * line, 8, 64, 255, "github.com/awardcat");
            uidisp->draw_printf(DISPX + DISPW - 16 - 6 * 21, DISPY - 4 + 16 * line + 8, 8, 64, 255, "/ExistOS-For-HP39GII");
            line++;
            uidisp->draw_printf(DISPX + (DISPW - (8 * 25)) / 2, DISPY - 4 + 16 * line, 8, 64, 255, "Forked from ExistOS-Team");
        }
    }
    if (isMsgBoxShow)
        msgbox->refresh();
...
void drawPage(int page) {
    ...
    case 0:
        ...
        // FormCalc 图标（48x48：与 RPN39 同尺寸同构——灰机壳 + 实心白表单纸，无框线元素）
        {
            int ix = mainw->content_x0 + 12 + 160, iy = mainw->content_y0 + 12;
            uidisp->draw_box(ix + 2, iy + 2, ix + 45, iy + 45, 45, -1);       // 机壳（RPN39 同款 44x44 灰）
            uidisp->draw_box(ix + 6, iy + 4, ix + 41, iy + 38, 255, -1);      // 白色表单纸（实心 36x35，无框）
            for (int r = 0; r < 3; r++) {                                     // 3 行输入字段：黑 label + 值灰条
                int y = iy + 8 + r * 7;
                uidisp->draw_box(ix + 8, y, ix + 12, y + 2, 0, -1);
                uidisp->draw_box(ix + 15, y, ix + 38, y + 2, 190, -1);
            }
            // 黑字 "F+" 从纸内伸出，底缘压过纸下缘（断口感）——纸底 38，字底 42
