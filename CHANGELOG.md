...
- 又按 v2 根因（`enterSlowDown` 先切 DCDC 轻载、后降 CPU 的"480MHz CPU + 轻载 DCDC"塌陷窗口）实施修复（boost 空闲不切 DCDC 轻载）——**实测仍卡**（兜底全关 + 开机直接加速的最严苛场景）→ 已回退。**最终结论：本机供电通路无法在电池下承载 480MHz（硬件退化），加速档仅限外接电源**
